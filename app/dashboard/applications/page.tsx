      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 md:gap-4 mb-6 md:mb-8">
          <StatCard
            title="Toplam"
            value={stats.total}
            icon={<FileText className="h-4 h-4 md:h-5 md:w-5" />}
            color="bg-blue-500"
          />
          <StatCard
            title="Beklemede"
            value={stats.beklemede}
            icon={<Clock className="h-4 h-4 md:h-5 md:w-5" />}
            color="bg-yellow-500"
          />
          <StatCard
            title="Hakem'de"
            value={stats.hakemde}
            icon={<AlertCircle className="h-4 h-4 md:h-5 md:w-5" />}
            color="bg-blue-500"
          />
          <StatCard
            title="Kabul"
            value={stats.kabul}
            icon={<CheckCircle className="h-4 h-4 md:h-5 md:w-5" />}
            color="bg-green-500"
          />
          <StatCard
            title="Red"
            value={stats.red}
            icon={<XCircle className="h-4 h-4 md:h-5 md:w-5" />}
            color="bg-red-500"
          />
          <StatCard
            title="Revizyon"
              ...
              <table className="w-full min-w-[800px]">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    ...
                    <th className="px-3 md:px-6 py-3 md:py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden sm:table-cell">
                      Tip
                    </th>
                    <th className="px-3 md:px-6 py-3 md:py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                      Durum
                    </th>
                    <th className="px-3 md:px-6 py-3 md:py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden md:table-cell">
                      Ödeme
                    </th>
                    <th className="px-3 md:px-6 py-3 md:py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden lg:table-cell">
                      Tarih
                    </th>
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-3 md:p-4">
      <div className="flex items-center gap-2 md:gap-3">
        <div className={`${color} w-8 h-8 md:w-10 md:h-10 rounded-lg flex items-center justify-center text-white shrink-0`}>
          {icon}
        </div>
  ...
  return (
    <tr className="hover:bg-gray-50 transition-colors">
      <td className="px-3 md:px-6 py-3 md:py-4">
        <div>
          <Link
            ...
            className="font-semibold text-sm md:text-base text-gray-900 hover:text-blue-600 line-clamp-1 block"
          >
            {application.event.baslik}
          </Link>
          {application.baslik && (
            <p className="text-xs md:text-sm text-gray-600 line-clamp-1 mt-1">
      ...
      <td className="px-3 md:px-6 py-3 md:py-4 whitespace-nowrap">
        <span className={`inline-flex items-center gap-1.5 px-2 md:px-3 py-1 md:py-1.5 text-xs font-medium rounded-full ${status.color}`}>
          {status.icon}
        ...
        <div className="flex items-center justify-end gap-2">
          ...
          {application.dosya_url && (
            <a
              ...
              className="inline-flex items-center gap-1 px-2 md:px-3 py-1 md:py-1.5 text-xs font-medium text-gray-600 hover:text-gray-700 hover:bg-gray-100 rounded-lg transition-colors"
            >
